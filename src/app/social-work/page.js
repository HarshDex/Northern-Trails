...
export const metadata = {
  title: 'Social Work & Community Impact | Northern Trails',
  description:
  ...
  alternates: { canonical: 'https://www.northerntrails.in/social-work' },
};

const commitments = [
  {
    title: 'Zero Waste on Every Route',
    body: 'Every camp generates waste. Every piece comes back down. We carry refillable water systems, ban single-use plastics, and brief every yatri on pack-in/pack-out before departure.',
    icon: '♻',
  },
  {
  ...
  {
    title: 'Community Conservation Fund',
      ...
      <main>
        ...
        >
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at 20% 0%, rgba(74,123,82,0.12) 0%, transparent 55%)' }}
          />
        ...
        <section className="bg-cream py-16">
          <div className="mx-auto max-w-3xl px-5 md:px-10">
            <p className="reveal text-[16px] md:text-[18px] leading-relaxed text-charcoal/70">
              ...
              Northern Trails was started specifically to operate differently. Here is exactly how.
            </p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl px-5 md:px-10">
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {commitments.map((c, i) => (
                <div
                  key={c.title}
                  className={`reveal reveal-d${(i % 3) + 1} rounded-2xl border border-border bg-cream-card p-7`}
                >
                  <div className="text-2xl mb-4" aria-hidden="true">{c.icon}</div>
                  <h3 className="font-display text-lg font-semibold text-charcoal mb-3">{c.title}</h3>
                  <p className="text-sm leading-relaxed text-charcoal/60">{c.body}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-5 md:px-10">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {impactStats.map((s, i) => (
                <div
                  key={s.label}
                  className={`reveal reveal-d${i + 1} rounded-2xl border border-border bg-cream-card p-7`}
                >
        ...
        >
          <div className="mx-auto max-w-xl px-5">
            ...
            <div className="reveal reveal-d2 flex flex-wrap justify-center gap-4">
              <Link href="/#packages" className="rounded-full bg-saffron px-7 py-3.5 text-sm font-semibold text-white hover:bg-saffron/90 transition-colors">
                View Our Yatras
              </Link>
